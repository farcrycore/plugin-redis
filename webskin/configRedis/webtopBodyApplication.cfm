...
<cfset qItems = redis.getItems(server=url.server, app=url.app, version=application.fc.lib.objectbroker.getCacheVersion()) />
<cfset stSizes = redis.getItemSizeStats(qItems) />
<cfset stTypes = redis.getItemTypeStats(qItems) />
<cfset stExpiries = redis.getItemExpiryStats(qItems,true) />
...
<cfoutput>
	<h1>Redis Status - #url.server# - #url.app#</h1>
